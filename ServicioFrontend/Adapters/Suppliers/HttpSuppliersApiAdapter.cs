...
using ServicioFrontend.Dtos.Suppliers;

namespace ServicioFrontend.Adapters.Suppliers;
...
public sealed class HttpSuppliersApiAdapter(IHttpClientFactory httpClientFactory) : ISuppliersApiAdapter
{
    ...
    private async Task<ApiResponseDto<T>> GetAsync<T>(string requestUri, CancellationToken cancellationToken)
    {
        try
        {
            ...
            return ApiResponseDto<T>.Fail("El servicio de proveedores no devolvió una respuesta válida.");
        }
        ...
        {
            return ApiResponseDto<T>.Fail("No se pudo conectar con el servicio de proveedores.");
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            return ApiResponseDto<TResponse>.Fail("El servicio de proveedores no devolvió una respuesta válida.");
        }
        catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)
        {
            throw;
        ...
        {
            return ApiResponseDto<TResponse>.Fail("No se pudo conectar con el servicio de proveedores.");
        }
    }
