...
namespace ServicioFrontend.Adapters.Employees;
...
public sealed class HttpEmployeesApiAdapter(IHttpClientFactory httpClientFactory) : IEmployeesApiAdapter
{
    ...
    private async Task<ApiResponseDto<T>> GetAsync<T>(string requestUri, CancellationToken cancellationToken)
    {
        try
        {
            ...
            return ApiResponseDto<T>.Fail("El servicio de empleados no devolvió una respuesta válida.");
        }
        ...
        {
            return ApiResponseDto<T>.Fail("No se pudo conectar con el servicio de empleados.");
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            return ApiResponseDto<TResponse>.Fail("El servicio de empleados no devolvió una respuesta válida.");
        }
        catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)
        {
            throw;
        ...
        {
            return ApiResponseDto<TResponse>.Fail("No se pudo conectar con el servicio de empleados.");
        }
    }
