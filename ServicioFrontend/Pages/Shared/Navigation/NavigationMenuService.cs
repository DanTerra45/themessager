...
{
    private readonly bool _salesEnabled;
    ...
    public NavigationMenuViewModel Build(ClaimsPrincipal user)
    {
        ...
        var isOperator = user.IsInRole("Operator") || user.IsInRole("Operador");
        ...
        if (isAuthenticated)
        {
            var commercialItems = BuildCommercialItems(isAdmin, isOperator, isAuditor, _salesEnabled);
    ...
    private static List<NavigationDropdownItem> BuildCommercialItems(bool isAdmin, bool isOperator, bool isAuditor, bool salesEnabled)
    {
        var items = new List<NavigationDropdownItem>();

        if (salesEnabled && (isAdmin || isOperator))
        {
        ...
        if (salesEnabled && (isAdmin || isOperator || isAuditor))
        {
            items.Add(new NavigationDropdownItem
            {
                Title = "Reportes",
