...
using ServicioFrontend.Dtos.Employees;
using ServicioFrontend.Dtos.Users;
using ServicioFrontend.Pages.Infrastructure;
using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.Mvc;

namespace ServicioFrontend.Pages.Users;

[Authorize(Policy = "AdminOnly")]
public sealed class IndexModel(
    IUsersApiAdapter usersApiAdapter,
    IEmployeesApiAdapter employeesApiAdapter,
    ILogger<IndexModel> logger) : FrontendPageModel
{
    private const int AvailableEmployeesPageSize = 50;
    ...
    public async Task<IActionResult> OnPostDeactivateAsync()
    {
        var result = await usersApiAdapter.DeactivateUserAsync(
            DeactivateUserId,
            new DeactivateUserRequestDto("Baja administrativa solicitada desde el frontend."),
            BuildActorContext(),
            HttpContext.RequestAborted);
    ...
    private async Task LoadPageDataAsync()
    {
        ...
        var employeesResult = await employeesApiAdapter.GetEmployeesAsync(
            AvailableEmployeesPageSize,
            sortBy: "apellidos",
            sortDirection: "asc",
            anchorEmployeeId: 0,
            cursorEmployeeId: 0,
        ...
        {
            AvailableEmployees = employeesResult.Data.Employees
                .Select(MapAvailableEmployee)
                .ToList();
            return;
        }
    ...
    private static AvailableEmployeeDto MapAvailableEmployee(EmployeeDto employee)
    {
        var fullName = string.Join(
            ' ',
            new[] { employee.Nombres, employee.PrimerApellido, employee.SegundoApellido }
                .Where(part => !string.IsNullOrWhiteSpace(part)));

        var ciDisplay = employee.Ci.ToString();
        if (!string.IsNullOrWhiteSpace(employee.Complemento))
        {
            ciDisplay = string.Concat(ciDisplay, "-", employee.Complemento.Trim().ToUpperInvariant());
        }

        return new AvailableEmployeeDto(
            employee.Id,
            string.IsNullOrWhiteSpace(fullName) ? "Sin nombre" : fullName,
            employee.Cargo,
            ciDisplay);
    }
