    ...
    function bindEditSupplierModal() {
        ...
        editModal.addEventListener('show.bs.modal', async function (event) {
            ...
            try {
                ...
                if (editCodigoInput) {
                    editCodigoInput.value = supplier.codigo;
                }

                if (editRazonSocialInput) {
                    editRazonSocialInput.value = supplier.nombre;
                }

                if (editDireccionInput) {
                    editDireccionInput.value = supplier.direccion;
                }

                if (editContactoInput) {
                    editContactoInput.value = supplier.contacto;
                }

                if (editTelefonoInput) {
                    editTelefonoInput.value = supplier.telefono || '';
                }

                if (editRubroInput) {
                    editRubroInput.value = supplier.rubro;
                }
            } catch (error) {
                console.error(error);
            }
