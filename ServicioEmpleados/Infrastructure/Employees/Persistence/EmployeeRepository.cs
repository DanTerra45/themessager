...
using ServicioEmpleados.Domain.Shared.Repository;
using Npgsql;
using System.ComponentModel.DataAnnotations;
using System.Data;
using System.Text;
using ServicioEmpleados.Domain.Shared.Exceptions;
using ServicioEmpleados.Domain.Shared.Validation;
using Microsoft.Extensions.Logging;

namespace ServicioEmpleados.Infrastructure.Employees.Persistence
{
    public class EmployeeRepository(
        IDbConnectionFactory dbConnection,
        ILogger<EmployeeRepository> logger) : IEmployeeRepository, ICrudRepository<Employee, Employee, EmployeeModel, long>
    {
        ...
        private static string BuildEmployeeRowsQuery(bool hasSearchTerm)
        {
            ...
            if (hasSearchTerm)
            {
                queryBuilder.Append(@"
                    AND (
                        CAST(ci AS TEXT) LIKE @SearchPattern
                        OR COALESCE(complemento, '') LIKE @SearchPattern
                        OR (CAST(ci AS TEXT) || CASE WHEN COALESCE(complemento, '') = '' THEN '' ELSE '-' || complemento END) LIKE @SearchPattern
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            catch (InvalidOperationException exception) when (exception.InnerException is PostgresException)
            {
                throw CreateDataStoreUnavailableException("consultar empleados", exception);
            }
        }
        ...
        public async Task<long> CreateAsync(Employee employee, CancellationToken cancellationToken = default)
        {
            ...
            catch (PostgresException exception)
            {
                _logger.LogError(
                    exception,
                    "Error PostgreSQL al guardar empleado. SqlState={SqlState}, Constraint={Constraint}, Detail={Detail}, Hint={Hint}",
                    exception.SqlState,
            ...
            catch (InvalidOperationException exception) when (exception.InnerException is PostgresException)
            {
                ...
                _logger.LogError(
                    postgresException,
                    "Error PostgreSQL envuelto al guardar empleado. SqlState={SqlState}, Constraint={Constraint}, Detail={Detail}, Hint={Hint}",
