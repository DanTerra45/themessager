...
namespace Mercadito.src.categories.data.repository
{
    public class CategoryRepository(IDataBaseConnection dbConnection) : ICategoryRepository
    {
        ...
        public async Task<IEnumerable<CategoryModel>> GetCategoryByPages(int page, int pageSize, CancellationToken cancellationToken = default)
        {
        ...
        public async Task<int> UpdateCategoryAsync(Category category, CancellationToken cancellationToken = default)
        {
         ...
         WHERE id = @Id AND estado = 'A'";
            return await connection.ExecuteAsync(query, new { category.Id, category.Code, category.Name, category.Description });
        }
