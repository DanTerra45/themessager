    ...
    where TSchema : class, ITableSchema<TFields>, new()
    {
        ...
        protected async Task<Result<IEnumerable<TEntity>>> ExecuteQueryAsync(string sql, DynamicParameters parameters, IDbTransaction? transaction = null)
        {
            if (transaction != null)
            {
                ...
                if (txConn is null)
                {
                    throw new InvalidOperationException("La transaccion no tiene conexion asociada.");
                }
        ...
        public virtual Task<Result<bool>> DeleteAsync(TOptions? options, CancellationToken cancellationToken = default)
        {
            throw new NotImplementedException();
        }
    }
